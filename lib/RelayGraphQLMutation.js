...
var RelayGraphQLMutation = function () {
  ...
  RelayGraphQLMutation.prototype.applyOptimistic = function applyOptimistic(optimisticQuery, optimisticResponse, configs) {
    !!this._transaction ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayGraphQLMutation: `applyOptimistic()` was called on an instance ' + 'that already has a transaction in progress.') : invariant(false) : void 0;
...
var PendingGraphQLTransaction = function () {
  ...
  PendingGraphQLTransaction.prototype.getCallName = function getCallName() {
    !this._mutation ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayGraphQLMutation: `getCallName()` called but no mutation exists ' + '(`getQuery()` must be called first to construct the mutation).') : invariant(false) : void 0;
  ...
  PendingGraphQLTransaction.prototype._getVariables = function _getVariables() {
    ...
    if (!input) {
      !false ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayGraphQLMutation: Required `input` variable is missing ' + '(supplied variables were: [%s]).', (0, _keys2['default'])(this._variables).join(', ')) : invariant(false) : void 0;
    }
