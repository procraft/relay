...
var RelayStoreReader = function (_RelayQueryVisitor) {
  ...
  RelayStoreReader.prototype._readPageInfo = function _readPageInfo(node, rangeInfo, state) {
    ...
    !pageInfo ? process.env.NODE_ENV !== 'production' ? invariant(false, 'readRelayQueryData(): Missing field, `%s`.', PAGE_INFO) : invariant(false) : void 0;
    if (rangeInfo.diffCalls.length) {
      state.isPartial = true;
    }
...
var RelayRangeCallEnforcer = function (_RelayQueryVisitor2) {
  ...
  RelayRangeCallEnforcer.prototype.visitField = function visitField(node, parent) {
    ...
    !(schemaName !== EDGES && schemaName !== PAGE_INFO) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'readRelayQueryData(): The field `%s` is a connection. Fields `%s` and ' + '`%s` cannot be fetched without a `first`, `last` or `find` argument.', parent.getApplicationName(), EDGES, PAGE_INFO) : invariant(false) : void 0;
  };
...
function getDataObject(state) {
  ...
  !(data instanceof Object) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'readRelayQueryData(): Unable to read field on non-object.') : invariant(false) : void 0;
  return data;
}
