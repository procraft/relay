...
var RelayCacheProcessor = require('./RelayCacheProcessor');
...
var RelayCachedDataRestorator = function (_RelayCacheProcessor) {
  ...
  RelayCachedDataRestorator.prototype.traverse = function traverse(node, nextState) {
    !(nextState.dataID != null) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayCachedDataRestorator: Attempted to traverse without a ' + '`dataID`.') : invariant(false) : void 0;
    ...
    for (var ii = 0; ii < pendingNodeStates.length; ii++) {
      ...
      !(pendingNodeStates[ii].dataID != null) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayCachedDataRestorator: Attempted to visit a node without ' + 'a `dataID`.') : invariant(false) : void 0;
